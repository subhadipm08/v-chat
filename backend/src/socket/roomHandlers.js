...
export const registerRoomHandlers = ({ io, socket, userId }) => {
  ...
  socket.on('join-room', wrapSocket(socket, async ({ roomId }) => {
  ...
  socket.on('verify-room', wrapSocket(socket, async ({ roomId }, callback) => {
    const room = await Room.findOne({ roomId });
    if (!room) {
  ...
  socket.on('leave-room', wrapSocket(socket, async ({ roomId }) => {
    await leavePrivateRoom(roomId, 'LEFT');
  }));

  socket.on('media-state-change', wrapSocket(socket, async ({ roomId, isMicOn, isCameraOn }) => {
    if (!roomId) {
      return;
    }
